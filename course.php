...
$topic_slug  = trim($_GET['s'] ?? '');
$course_slug = trim($_GET['c'] ?? '');

// Fallback: hanya boleh diakses lewat course.php?s=... (topic) atau course.php?c=... (course, arahkan ke topic pertama)
if ($topic_slug === '' && $course_slug === '') {
    header('Location: ./');
    exit;
}

// Entry point via course slug -> cari topic pertama lalu redirect ke URL topic (bookmarkable & konsisten)
if ($topic_slug === '' && $course_slug !== '') {
    $courseStmt = $pdo->prepare("SELECT id, title FROM courses WHERE slug = ? AND is_published = 1 LIMIT 1");
    $courseStmt->execute([$course_slug]);
    $activeCourse = $courseStmt->fetch();

    if (!$activeCourse) {
        render_reader_message(
            'bx-error-circle',
            'Course tidak ditemukan',
            'Course yang kamu cari mungkin sudah tidak aktif atau URL-nya salah.'
        );
    }

    $firstTopicStmt = $pdo->prepare("
        SELECT t.slug
        FROM topics t
        JOIN modules m ON t.module_id = m.id
        WHERE m.course_id = ?
        ORDER BY m.order_position ASC, m.id ASC, t.order_position ASC, t.id ASC
        LIMIT 1
    ");
    $firstTopicStmt->execute([$activeCourse['id']]);
    $firstTopic = $firstTopicStmt->fetch();

    if (!$firstTopic) {
        render_reader_message(
            'bx-folder-open',
            'Belum ada materi',
            'Course "' . htmlspecialchars($activeCourse['title']) . '" belum memiliki topic/materi pembelajaran.'
        );
    }

    header('Location: ./course?s=' . rawurlencode($firstTopic['slug']));
    exit;
}

/**
 * Menampilkan halaman pesan sederhana (course tidak ditemukan / belum ada materi / topic tidak ditemukan)
 * dengan gaya visual yang konsisten dengan reader, lalu menghentikan eksekusi.
 */
function render_reader_message(string $icon, string $title, string $message): void {
    ...
    <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Merriweather:wght@700&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
        <style>
            body { background: #faf8f5; font-family: 'Plus Jakarta Sans', sans-serif; }
            h1 { font-family: 'Merriweather', serif; }
        </style>
    </head>
    <body class="min-h-screen flex items-center justify-center px-6">
        <div class="max-w-sm w-full text-center">
            <div class="w-16 h-16 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center mx-auto mb-5">
                <i class="bx <?= htmlspecialchars($icon) ?> text-3xl"></i>
            </div>
            <h1 class="text-xl font-bold text-slate-900 mb-2"><?= htmlspecialchars($title) ?></h1>
            <p class="text-sm text-stone-500 leading-relaxed mb-6"><?= htmlspecialchars($message) ?></p>
            <a href="./" class="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-indigo-600/25 transition-all">
                <i class="bx bx-arrow-back"></i> Kembali ke Katalog Course
            </a>
        </div>
    </body>
    </html>
...
function topic_url(array $item): string {
    return './course?s=' . rawurlencode($item['slug']);
}
...
if (!$topic) {
    render_reader_message(
        ...
        'Materi yang kamu cari mungkin sudah dipindahkan atau URL-nya salah.'
    );
}
?>
<!DOCTYPE html>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="sidebar">
            <div class="h-[72px] border-b border-stone-200 px-5 flex items-center">
                ...
                <span class="min-w-0 ms-2">
                    <a href="./" title="Kembali ke halaman utama">
                        <span class="flex items-center gap-1 text-[10px] font-bold uppercase tracking-[.16em] text-stone-400">
                            <i class="bx bx-chevron-left text-xs"></i> Kembali ke Home
                        </span> 
                    </a>
            ...
            <header class="reader-header app-font">
                <div class="h-full px-4 sm:px-6 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="truncate text-[10px] uppercase tracking-[.16em] font-bold text-stone-400">
                                <?= htmlspecialchars($topic['module_title']) ?>
                            </div>
                            <div class="truncate max-w-[78vw] sm:max-w-[80vw] text-xs sm:text-sm font-bold text-slate-800">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <img src="./assets/img/aguphia-icon.png" class="w-10 h-10">
                        <!--
                        <button
                            type="button"
                            onclick="toggleFont()"
